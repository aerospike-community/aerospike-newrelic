...
public class Base {
	 ...
	 * @param ip IP of the Aerospike node
	 * @param port Port number of Aeropsike node
	 * @param user User name for Aerospike node if security enabled else null
	 * @param password Password for Aerospike node if if security enabled else null
	 */
	public void createAerospikeClient(String ip, Integer port, String user, String password){
		...
		if(this.client == null && this.policy == null){
			...
			if (this.client == null || !this.client.isConnected()) {
				System.out.println("ERROR: Connection to Aerospike cluster failed! Please check the server settings and try again!");
			}
		}
	}
	 ...
	 * @param ip IP of the Aerospike node
	 * @param port Port number for Aerospike node
	 * @param user User name for Aerospike node if security enabled else null
	 * @param password Password for Aerospike node if security enabled else null
	 ...
	 * @param ip IP of Aerospike node
	 * @param port Port number of Aerospike node
	 * @param user User name for Aerospike if security enabled else null
	 * @param password Password for Aerospike if security enabled else null
	 * @return Map<String, String> Map of namespace statistics
	 */
	public Map<String, String> getNamespaceStatistics(String namespace ,String ip, Integer port, String user, String password){
	...
	public Map<String, Map<String, String>> getNodeLatency(String ip, Integer port, String user, String password){
		...
		for(Integer i = 0; i < latencyBuckets.length; i +=2 ){
			...
			for(Integer j = 0; j < keys.length; j++){
				...
				String k = "";
				if(previousKey != ""){
					k = "greater than " + keys[j].substring(1) + " to less than " + previousKey.substring(1);
				}
				else{
					k = "greater than " + keys[j].substring(1);
				}
				previousKey = keys[j];
				lessThan1 -= pct;
			...
			data.put("less than 1ms", String.valueOf(lessThan1Val) + ";" + String.valueOf(lessThan1));
	...
	public Map<String, Map<String,String>> getThroughput(Map<String, String> nodeStats){
		...
		String oldReadReqs, oldReadSuccess, oldWriteReqs, oldWriteSuccess, newReadReqs, newReadSuccess, newWriteReqs, newWriteSuccess;
		...
		boolean writeCondition = newWriteReqs.isEmpty() || newWriteSuccess.isEmpty() || oldWriteReqs.isEmpty() || oldWriteSuccess.isEmpty();
		boolean readCondition = newReadReqs.isEmpty() || newReadSuccess.isEmpty() || oldReadReqs.isEmpty() || oldReadSuccess.isEmpty();
	...
	public Map<String, String> getMemoryStats(Map<String, String> nodeStats){
		...
		System.out.println("used bytes memory: " + nodeStats.get("used-bytes-memory"));
		System.out.println("total bytes memory: " + nodeStats.get("total-bytes-memory"));
		try{
			...
			Float freeBytes = (Float.parseFloat(nodeStats.get("total-bytes-memory")) - Float.parseFloat(nodeStats.get("used-bytes-memory")));
	...
	public Map<String, String> getDiskStats(Map<String, String> nodeStats){
		...
		try{
			...
			Float freeBytes = (Float.parseFloat(nodeStats.get("total-bytes-disk")) - Float.parseFloat(nodeStats.get("used-bytes-disk")));
